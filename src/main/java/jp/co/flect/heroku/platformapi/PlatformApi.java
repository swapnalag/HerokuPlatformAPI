...
public class PlatformApi implements Serializable {
	...
	public static PlatformApi fromPassword(String username, String password) throws IOException {
		...
		request.setHeader("Accept", "application/vnd.heroku+json; version=3.43.3");
	...
	public Formation updateFormation(String appName, String idOrName, int quantity, String size) throws IOException {
	...
	public Dyno runDynoSize(String appName, Dyno dyno) throws IOException {
		HttpRequest request = buildRequest(HttpRequest.Method.PUT, "/apps/" + appName + "/dynos/" + dyno.getType());
		request.setParameter("command",dyno.getCommand());
		System.out.println("Dyno Size in PlatForm Api: " + dyno.getSize());
		request.setParameter("size",dyno.getSize());
		System.out.println("Request in PlatForm Api: " + request.getParameters());
		HttpResponse res = getTransport().execute(request);
		return handleResponse("runDynoSize", res, Dyno.class).get(0);
	}
